...
3. [RS0x Private CAN Protocol](#3-rs0x-private-can-protocol)
   ...
   - [Parameter Read / Write (types 17 & 18)](#35-parameter-read--write-types-17--18)
...
| 17 | 0x11 | Host → Motor | Single parameter read |
| 18 | 0x12 | Host → Motor | Single parameter write (volatile) |
| 21 | 0x15 | Motor → Host | Fault feedback |
...
### 3.5 Parameter Read / Write (types 17 & 18)
...
│  Notifier + per-motor BufferedReader queues, bandwidth stats │
└─────────────────────┬────────────────────────────────────────┘
                      │
              SocketCAN (can0)
                      │
...
Each motor that is registered via `add_motor_filter(motor_id)` installs a
...
This means frames for unregistered motors are dropped by the driver, not
Python, keeping CPU load low.

### Background listener

`start_listener()` starts a `can.Notifier` background thread that feeds a
`_MotorDispatcher`.  The dispatcher routes each arriving frame into the
correct motor's private `can.BufferedReader` queue based on the motor ID
in bits 15–8 of the extended arbitration ID.
...
_MotorDispatcher.on_message_received(msg)
    ├── bandwidth counter
    ├── motor.callback(msg)      ← updates motor._feedback immediately
    └── motor.BufferedReader     ← queued for recv_for_motor()
```

This means `motor.feedback` is **always current** even if the caller never
explicitly reads it.
...
```python
bus.bandwidth()
# CANComms bandwidth (1.00 s window):
#   TX     600 frames     78.60 kbps
#   RX     600 frames     78.60 kbps
#   Bus load: 15.7% of 1000 kbps
```
...
use_node_name_as_topic_base = true          # false → topics at root namespace (e.g. /joint_states)
master_id                   = 253           # host CAN ID (0xFD)
channel                     = "can0"        # SocketCAN interface
bustype                     = "socketcan"
bitrate                     = 1000000
rx_timeout                  = 0.05          # seconds
active_report_interval_ms   = 10            # minimum 10 ms
...
The `[defaults]` section is **required** — the node exits with a fatal error if it
is absent.
...
| `~/motors/{name}/fault` | `custom_interfaces/MotorFault` | Per-motor decoded fault bits — published **only on change** |
...
# Change baud rate on all motors
ros2 service call /motor_node/set_can_config custom_interfaces/srv/SetCanConfig \
  '{name: all, can_id: 0, baud_flag: 1}'
...
ros2 action send_goal /motor_node/set_velocity \
  ...
  '{name: left_hip, target_velocity: 3.0, duration: 5.0, current_limit: 20.0}'
```

Velocity is ramped to 0 when duration expires or the goal is cancelled.
...
| `MotorFault` | `name`, `fault_code`, decoded per-bit booleans |

**Services** (`srv/`): `EnableMotor`, `SetRunMode`, `SetZeroPosition`, `ReadParam`, `WriteParam`, `Help`, `GetCanConfig`, `SetCanConfig`
...
# Build interfaces first (robstride_p depends on them)
...
with CANComms("can0") as bus:
    bus.start_listener()            # background Notifier thread
    ...
    for _ in range(100):
        hip.spin_once()
        knee.spin_once()
        print(hip.feedback, knee.feedback)
        time.sleep(0.01)
...
```
claude/
├── comms.py          CAN transport layer (CANComms, _MotorDispatcher)
...
│       │   ├── msg/  MotorState.msg  MotorCommand.msg  MotorFault.msg
│       │   ├── srv/  EnableMotor.srv  SetRunMode.srv  SetZeroPosition.srv
│       │   │         ReadParam.srv  WriteParam.srv  Help.srv
│       │   └── action/  MoveToPosition.action  SetVelocity.action
...
│               ├── rs01.py … rs05.py
│               └── motor_node.py
└── RS01.pdf … RS05.pdf  Official user manuals
```
